...
- Undo of the edit you just made, and Undo/Redo back and forth, show on the timeline at once instead of after a round trip to the server (about half a second); the first Undo of a session, and older steps in a row of Undos, still wait for it. If the project changed elsewhere in the meantime, for example a generated take landed, the other change appears once the Undo has saved; nothing is lost.
