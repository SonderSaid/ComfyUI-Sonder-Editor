...
- The Reference Library responds at once: a new Reference, a new member and Up/Down show immediately, marked "Saving…" until saved, and the create form closes on Save. A change that is refused disappears again and hands its form back, unless another Library form is open by then; one whose answer never arrives stays marked "Not confirmed" until the Library can be read again. Pressing Up or Down several times in a row now applies every press instead of refusing the second, and the Library no longer blanks to "Loading references…" while it refreshes.
- Undo of the edit you just made, and Undo/Redo back and forth, show on the timeline at once instead of after a round trip to the server (about half a second). The first Undo of a session, older steps in a row of Undos, and an Undo after something else changed the project still wait for it.
...
- The Metadata Collector records a wired subgraph by name with the values of its inputs, instead of only the inner node that produced the output; the producing node's other values and any Power LoRA on that branch follow as their own sections.
- Asset Gallery details now offer Copy on read-only fields, prompts, names, paths, raw metadata, and Where Used information, with keyboard access and Power LoRA row JSON. Right-click copies anywhere; in Compare, where right-click filters B, a Copy button appears on hover instead.

### Fixed
- Copy on a capped tracked-metadata field now copies the complete saved value. On an older asset that cannot isolate the field, it says it is copying the whole section's raw text.
...
### Added
- The gallery preloads generation metadata. Inspecting an asset loads its details without reloading the asset list, and the next 50 assets in your direction of travel (gallery list, inspector, or the active compare side) load in the background, as do assets that arrive while a gallery is open. Three Asset Gallery settings control this: **Preload Following Assets**, **Preload New Assets** and **Max Cached Provenance Details**.

### Changed
- MiniMax H3 Summary task types are checkboxes in the identity editor, the Attach dialog and the chip editor. Each check shows where it comes from (an identity, a member, or the staged Roles), and opening or saving a dialog keeps an automatic choice automatic. **Customize** makes the checks editable; **Reset to inherited** returns to automatic. Attach and the chip editor also show the scene-wide task types from the last compile.
- `tracked:` and `field:` searches, in the gallery and both compare pickers, no longer reload the asset list and reuse metadata already loaded. A search waits until every asset is searchable, showing that it is loading, and offers Retry if loading fails.
- **⌇ Split Here** acts only on the selection and now also splits prompt sections. With nothing selected it shows a message. Splitting several selected items is one Undo step.
- Splits, typed-frame moves, moves to a new lane, consolidating onto one lane, Reference staging, link badges and predictable Undo/Redo steps now appear on the timeline before the save finishes. A failed edit restores the timeline.
- Repeated clicks on one lane header's hide control, mute toggles on one selection, and clip-role conversions share one save per save already running. Clicks on different lanes remain separate Undo steps.
- Project reads and saves, including the first automatic format migration, run on their own worker threads, so thumbnails, probes and uploads no longer wait behind them. Setting a scene option to its current value no longer saves the project.
- When a save collides with a running render, the editor retries only edits that name their target by an identity the server checks. An edit that names a lane, section or position by its place in a list is refused and the timeline restored, so it cannot land on the wrong lane.

### Removed
- The standalone `DELETE` routes for clips, guides, prompt sections and audio tracks, and the dedicated guide-swap route. Use the scene mutations endpoint, which checks the target's identity and keeps link groups intact.

### Fixed
- The Prompt, Guides and Reference Lane panels act on the wrong item after Undo. They now repaint when Undo, Redo or a save changes what they show, but not while a field in them has focus, and an action on an item that changed since the panel was drawn is refused with a message.
- Delete, Space and other timeline shortcuts reached the timeline behind an open Prompt, Guides or Reference Lane panel. Only Undo and Redo pass through now, and Escape closes the Guides panel without leaving fullscreen.
- Enter and Shift+Enter in the prompt editors insert the line break at the caret. In the Writing draft, Enter lost the break, moved the caret, or turned a prompt link into plain text. Deleting the last line's text no longer leaves a blank line.
- Undo got stuck on "Scene changed elsewhere" after locking several lanes quickly, toggling one setting during a save, renaming a lane, changing a Reference lane's recipe, or a take landing on a new lane. Lane renames and recipe changes are now Undo steps, a take's lane is kept, and Escape while renaming a lane discards the name.
- Fullscreen Undo/Redo after clicking an editor button could advance ComfyUI's graph history, including in subgraph editors.
- Guide swaps can be undone and redone, and a refused swap keeps the Redo history.
- Undo on a scene that already has items past the scene end, or overlapping on a lane, leaves those items as they are. A refused Undo names the rule, items, lanes and frames.
- A second cut made before the first finished saving did nothing; three quick cuts on one clip now give four pieces. A cut outside an item, on a locked lane, on a Driver clip or on empty space shows a message, a partly-crossing selection splits the items it can and names the rest, and splitting a prompt section refreshes the Prompt tool.
- A move, trim or delete on a just-split clip or audio track, made before the split finished saving, reached only that piece and left its linked partner behind. It now applies to the whole linked group.
- A second Library member dropped onto a Reference item that was still saving was refused and lost. It now appends to the item.
- Deleting a lane or a linked selection, adding a guide or dragging one onto an occupied frame, Writing-mode Apply, Reference staging, and linking a row an Undo moved are refused with a message when the target changed first elsewhere (a render writing the project, or another editor). Replacing a guide you can see works as before.
- Deleting or duplicating a scene, or deleting a saved selection, while a render writes to the project could discard the render's change. If the connection drops before the answer, the editor now reports that the outcome is unconfirmed.
- Emptying the trash or permanently deleting assets removed the media before the change was saved. If the save fails now, the files are put back and the assets stay in the trash.
- A finished timeline export could be deleted when its registration failed or it was cancelled late. The file is kept, and the message names it and explains how to add it back from the gallery.
- A failure to read the project's stored data was reported as a bad request containing the server's folder path, which could also be written into the project file. It is now a server error with a plain message. One case still shows the path: an export that fails after its file was saved.
- Saving a prompt identity stored an empty Summary task-type list when none was chosen, so its attachments ignored task types set on its physical References. An identity saved that way keeps the list until you press **Reset to inherited**.
- One asset whose metadata failed to load stopped the others loaded with it. Each now loads independently and offers Retry on failure.
- Sonder Cluster lost its saved lane and branch counts, and moved connections off their named sockets, when loading or switching workflows in newer ComfyUI frontends.
