...
from server.timeline_state import (
    ...
    ensure_default_prompt_semantic_units,
)
...
def test_default_subject_unit_tracks_separate_entity_member_mutations(tmp_path):
    project = _project(tmp_path)
    reference = ReferenceEntity(reference_id="ref-1", name="Granny", members=[])
    project.references = [reference]

    ensure_default_prompt_semantic_units(project)
    assert project.prompt_semantic_units == [{
        "semantic_unit_id": "unit:ref-1", "name": "Granny", "order": 0,
        "source_members": [], "visual_intent": "preserve",
        "audio_intent": "reference_characteristics", "definition": "",
        "intent_overrides": {},
    }]

    reference.members.append(ReferenceMember(
        member_id="member-1", asset_id="image-1", name="portrait"))
    ensure_default_prompt_semantic_units(project)
    reference.name = "Grandmother"
    reference.visual_intent = "partial"
    reference.audio_intent = "copy_full"
    reference.members.append(ReferenceMember(
        member_id="member-2", asset_id="audio-1", name="voice"))
    # Simulate the duplicate generated by the former empty-unit flow.
    project.prompt_semantic_units.append({
        **project.prompt_semantic_units[0],
        "source_members": [{"entity_id": "ref-1", "member_id": "member-1"}],
    })
    ensure_default_prompt_semantic_units(project)

    assert len(project.prompt_semantic_units) == 1
    unit = project.prompt_semantic_units[0]
    assert unit["semantic_unit_id"] == "unit:ref-1"
    assert unit["name"] == "Grandmother"
    assert unit["visual_intent"] == "partial"
    assert unit["audio_intent"] == "copy_full"
    assert unit["source_members"] == [
        {"entity_id": "ref-1", "member_id": "member-1"},
        {"entity_id": "ref-1", "member_id": "member-2"},
    ]


def test_default_subject_units_are_minted_only_for_subject_class_references(tmp_path):
    project = _project(tmp_path)
    project.references = [
        ReferenceEntity(
            reference_id="subject-ref",
            name="Lead",
            kind="character",
            reference_class="subject",
        ),
        ReferenceEntity(
            reference_id="context-ref",
            name="Apartment",
            kind="location",
...
def test_reference_class_flip_preserves_existing_generated_unit_and_mints_on_return(tmp_path):
    project = _project(tmp_path)
    ...
    reference.reference_class = "context"
    reference.name = "Lead, revised"
    ensure_default_prompt_semantic_units(project)

    assert len(project.prompt_semantic_units) == 1
    assert project.prompt_semantic_units[0]["semantic_unit_id"] == "unit:ref-1"
    assert project.prompt_semantic_units[0]["name"] == "Lead, revised"

    # Explicitly deleting the generated unit while the Reference is context
    # class must not recreate it. Returning to subject class mints it again.
    project.prompt_semantic_units = []
    ensure_default_prompt_semantic_units(project)
    assert project.prompt_semantic_units == []

    reference.reference_class = "subject"
    ensure_default_prompt_semantic_units(project)
    assert [unit["semantic_unit_id"] for unit in project.prompt_semantic_units] == [
        "unit:ref-1"
    ]
...
def test_reference_payload_marks_generated_subjects_without_persisting_provenance(tmp_path):
    project = _project(tmp_path)
    project.references = [ReferenceEntity(reference_id="ref-1", name="Granny", members=[])]
    ensure_default_prompt_semantic_units(project)
    project.prompt_semantic_units.append(prompt_context.normalize_semantic_unit({
        "semantic_unit_id": "unit:authored", "name": "Pair",
        "source_members": [], "generated": True,
    }))

    payload = routes._references_payload(project)

    assert [(value["semantic_unit_id"], value["generated"])
            for value in payload["prompt_semantic_units"]] == [
        ("unit:ref-1", True), ("unit:authored", False)]
